...
import copy
import random 
from utils import MarginRankingLoss_learning_loss
    ...
    def train(self, querry_dataloader, val_dataloader, task_model,LN, unlabeled_dataloader,lr_input,mode,iter):
        ...
        if mode==0:
            ...
            for iter_count in range(self.args.train_iterations):
                ...
                m_backbone_loss = torch.sum(target_loss) / target_loss.size(0)
                m_module_loss   =  self.LossPredLoss(pred_loss, target_loss) #loss_pred_criterion(pred_loss, target_loss) 
        ...
        #        final_accuracy=0
        #        stop=1
        return final_accuracy,task_model,LN,stop

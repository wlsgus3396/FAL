        ...
        if mode==0:
            ...
            for iter_count in range(self.args.train_iterations):
                ...
                for count in range(self.args.num_vae_steps):
                    ...
                    dsc_loss = self.bce_loss(np.squeeze(labeled_preds), lab_real_preds) + self.bce_loss(np.squeeze(unlabeled_preds), unlab_real_preds)
                ...
                for count in range(self.args.num_adv_steps):
                    ...
                    dsc_loss = self.bce_loss(np.squeeze(labeled_preds), lab_real_preds) + self.bce_loss(np.squeeze(unlabeled_preds), unlab_fake_preds)

...
def main(args):
    ...
    for iiter in range(args.global_iteration1): 
        ...
        Itask_model2=resnet.resnet18()
        num_ftrs = Itask_model2.linear.in_features
        Itask_model2.linear=nn.Linear(num_ftrs,args.num_classes)
        ...
        for k in range(args.num_clients):    
            ...
            FC2[k].load_state_dict(Itask_model2.linear.state_dict())
    ...
    f = open('./results/{}0_numclients_{}_lr_{}_lr_decay_{}_Fedacc_{}.csv'.format(args.execute,args.num_clients,args.lr,args.lr_decay,args.K),'w')
    f.write(A)
    f.close()

    Solo_accuracy=np.array(Solo_accuracy)
    A ="\n".join(map(str, Solo_accuracy))
    f = open('./results/{}0_numclients_{}_lr_{}_lr_decay_{}_Soloacc_{}.csv'.format(args.execute,args.num_clients,args.lr,args.lr_decay,args.K),'w')
    f.write(A)
    f.close()

...
def main(args):
    ...
    for iiter in range(args.global_iteration1): 
        ...
        if iiter!=args.global_iteration1-1:
            for k in range(args.num_clients):
                arg = solver[k].sample_for_labeling(task_model[k],FC1[k], FC2[k],unlabeled_selected_dataloader[k],querry_dataloader[k],untrain_dataset,unlabeled_indices[k],labeled_indices[k],len(labeled_indices[k]))

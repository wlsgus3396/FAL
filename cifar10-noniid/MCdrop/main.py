...
def main(args):
    ...
    for iiter in range(args.global_iteration1): 
        ...
        ###################################################################################
        for k in range(args.num_clients):
            DL_dis+=solver[k].DLcal(task_model,FC1, FC2,querry_dataloader[k])
            Num_dis+=len(labeled_indices[k])
            DL_item.append(solver[k].DLcal(task_model,FC1, FC2,querry_dataloader[k])/len(labeled_indices[k])/args.num_classes)
        
        DL_dis=DL_dis/Num_dis/args.num_classes
                            
        t_sampled_indices = solver[0].sample_for_labeling(task_model,FC1, FC2, DL_dis ,DL_item[0] ,t_unlabeled_dataloader)
